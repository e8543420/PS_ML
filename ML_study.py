...
if __name__ == "__main__":
    ...
    for col in FEM_parm:
        FEM_parm_cats[col] = pd.cut(FEM_parm[col], bins, labels=['lower','in','higher']).cat.codes
    for col in test_parm:
        test_parm_cats[col] = pd.cut(test_parm[col], bins, labels=['lower','in','higher']).cat.codes
...
#    my_pipeline = grid
    ...
    mesh_in = np.concatenate((mesh_in,np.ones((mesh_in.shape[0],18))*X.mean(axis=0)[2:]),axis=1)
